...
import { ModuleRegistry } from './modules.js'; // pokud modul: odstranit import a zajistit global
import { UserConfig } from './userConfig.js';

// Protože prohlížeče neumí importy defaultně, použijeme direct globals:

// Pomocná fallback, pokud import selže
window.ModuleRegistry = window.ModuleRegistry || ModuleRegistry;
window.UserConfig = window.UserConfig || UserConfig;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const saveUIBtn = document.getElementById('saveConfigUI');
  const panel = document.getElementById('configPanel');

  function renderPanel() {
    panel.innerHTML = '';
    ModuleRegistry.getAll().forEach(mod => {
      const checked = cfg.active.includes(mod.id) ? 'checked' : '';
      panel.insertAdjacentHTML('beforeend', `
        <div class="form-check">
          <input class="form-check-input" type="checkbox" id="chk_${mod.id}" ${checked}>
  ...
  loadBtn.addEventListener('click', async () => {
    container.innerHTML = '';
    if (!cfg.gasUrl) {
      return alert('Nastav URL backendu');
    }
